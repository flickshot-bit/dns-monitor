...
changesChart = new Chart(document.getElementById('changesChart'), {
    ...
    options: {
        ...
        scales: {
            x: { ticks: { color: '#555', font: { size: 9 } }, grid: { color: '#222' } },
            y: { ticks: { color: '#555', font: { size: 9 } }, grid: { color: '#222' }, beginAtZero: true }
        }
    }
});
...
typesChart = new Chart(document.getElementById('typesChart'), {
    type: 'doughnut',
    data: {
        labels: ['A', 'MX', 'NS', 'CNAME'],
        datasets: [{
            data: [0, 0, 0, 0],
            backgroundColor: ['#00bcd4', '#4caf50', '#ce93d8', '#ff9800'],
            borderColor: '#1d232a',
            borderWidth: 3
        }]
    },
    options: {
        ...
        plugins: {
            legend: {
                position: 'right',
                labels: { color: '#888', font: { size: 11 }, padding: 12, boxWidth: 12 }
            }
        }
    }
});
    ...
    fetch('/api/data')
        ...
        .then(function(data) {
            ...
            if (data.changes.length === 0) {
                cb.innerHTML = '<tr><td colspan="4" class="text-center opacity-30 py-4">No changes yet</td></tr>';
            } else {
